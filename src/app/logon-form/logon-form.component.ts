...
export class LogonFormComponent implements OnInit {
  ...
  async getRetrieveBooking(){
    this.bookingCode=this.retrieveBookingForm.get('bookingCode')?.value.toUpperCase();
    this.familyName=this.retrieveBookingForm.get('familyName')?.value.toUpperCase();


    const result= await this.bookingService.getBookingDetails(this.bookingCode,this.familyName);
